...
function LCSMemoization(s1, s2, i, j, dp) {
    ...
    if (s1[i] === s2[j]) 
        ...
        dp[i][j] = Math.max(LCSMemoization(s1, s2, i - 1, j, dp), LCSMemoization(s1, s2, i, j - 1, dp));
...
function main() {
    ...
    console.log("Length of LCS (Brute Force):", LCS(s1, s2, s1.length - 1, s2.length - 1));
    
    // 2. Memoization
    const dp = Array.from({ length: s1.length }, () => Array(s2.length).fill(-1));
    console.log("Length of LCS (Memoization):", LCSMemoization(s1, s2, s1.length - 1, s2.length - 1, dp));
    
    // 3. Tabulation
    console.log("Length of LCS (Tabulation):", LCSTabulation(s1, s2));
    
    // 4. Space Optimized Tabulation
    console.log("Length of LCS (Tabulation Space Optimized):", LCSTabulationSpaceOptimized(s1, s2));
